...
/// <summary>
/// Registers the co-op UNROPE shortcut on CrossMenuLib's LT+RT radial wheel: hold LT+RT and flick UP to
...
/// isn't roped to anyone. Registration is safe before the HUD exists; CrossMenuLib drives it once the wheel is
/// found.
///
/// CrossMenuLib glue, kept out of the MelonMod root so the orchestration layer is wiring-only.
/// </summary>
internal static class RopeWedge
{
    public static void Register(GameDriver driver, Action<string> log)
    {
        try
        {
            CrossMenuLib.CrossMenu.DefineMenu("cairncoop.rope", CrossMenuLib.Modifier.RightTrigger);
            CrossMenuLib.CrossMenu.Register(new CrossMenuLib.CrossMenuAction
            {
                Id = "cairncoop.rope.unrope",
                Label = "Unrope",
                Menu = "cairncoop.rope",
                Direction = CrossMenuLib.MenuDir.Up,
        ...
        {
            log?.Invoke("rope: CrossMenu registration failed (is CrossMenuLib installed?): " + e.Message);
        }
    }
}
